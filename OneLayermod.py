...
def forward_propagation( C, W, b, Z, Y, dt, lam, sigma ):
    ...
    if np.linalg.norm(diff) < gap:
        diff =  np.signe(RZ  - Y) * gap/2      # dimension:  d_y, n_z
...
Z =  np.linspace( 0 , 5.0, 20 )
Z =  Z.reshape(1, 20)               
Y =  np.where((Z>3) & (Z<4),1,0)

ZV = np.random.rand(1, 100) * 5
YV = np.where((ZV>2) & (ZV<4),1,0)

n_n=300

(C, W, b, costs) = nn_model( Z, Y, ZV, YV,  n_n, minibatch_size = 20, dt =0.01, lam = 0.01, sigma = 0.1, num_epochs =2000)
